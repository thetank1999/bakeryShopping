...
public class GetOrderDetailController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String idString = request.getParameter("orderId").replace("OD-", "");
        int id = Integer.parseInt(idString);
        System.out.println(id);

        try {
            OrderDAO orderDao = new OrderDAO();
            OrderSaleDTO order = orderDao.getOrderById(id);
            request.setAttribute("ORDER_INFO", order);

            OrderItemDAO dao = new OrderItemDAO();
            dao.getOrderDetail(id);
            List<OrderDetailDTO> orderDetai = dao.getOrderDetailList();
            request.setAttribute("ORDER_DETAIL_INFO", orderDetai);
            url = ORDER_DETAIL_PAGE;
        } catch (SQLException ex) {
            log("GetOrderDetailController_SQL: " + ex.getMessage());
        } finally {
